...
- **`run_sector_report_pipeline.py` & `cron_weekly_briefing.sh`**:
  - 매주 일요일 정기적으로 실행되며, KRX 전 종목 시세 데이터를 Parquet 형태로 캐싱하여 대규모 데이터를 빠르게 로드합니다.
  - Gemini 2.5 Flash를 활용해 5대 주도 섹터의 트렌드, 외인/기관 수급, 주요 테마 및 관련주를 심층 분석하여 `_위클리브리핑.md` 리포트를 발행합니다.
- **`generate_index.py`**:
  - 데일리 뉴스 파이프라인에서 생성된 `reports/` 리포트들과 `master_schedule_db.csv`의 단기 일정을 읽어옵니다.
  - 대시보드 메인 배너 상단에 `📖 사용법 가이드(guide.html)` 바로가기 링크를 제공하여 첫 방문자도 손쉽게 정보에 접근할 수 있게 지원합니다.
  - 메인 레이아웃을 `티커배너` ➔ `🌟 핵심주도이벤트` ➔ `⚡ 실시간최신통합포털(탭전환: 📅 주간캘린더 default / 📈 추천매매종목 / 🔥 기관리포트)` ➔ `📂 뉴스아카이브` 순서로 직관적으로 조립합니다.
  - `broker_upgrades.csv`를 조회하여 당일 목표가 상승률 상위 리포트들을 제목 미등록 제외 및 동일 종목 중복 제거 후 **`종목명 | 증권사 | 상승률(%)` 3개 컬럼 표** 형태로 깔끔하게 렌더링합니다.
  - `실전플랜 1` 스크리닝 배너를 생성할 때, 하드코딩된 JSON을 읽는 대신 **가장 최신의 `_스크리닝.md` 리포트를 직접 실시간으로 파싱**하여 종목 추천 목록을 동적으로 구성합니다.
  - `위클리브리핑` 필터 클릭 시 자바스크립트를 이용해 현재 월과 주차(예: 8월 2주차)를 자동 연산하여 직관적인 뱃지 형태로 렌더링합니다.
...
3. **[3단계 하이브리드 뉴스 수집 & 정제 파이프라인 (3-Stage News Pipeline)]**:
   - **1단계 (풍부한 수집)**: 네이버 API 및 해외 RSS 수집 시 기사 `Title + Description 150자`를 결합하여 1차 임베딩 라우팅을 진행함으로써 풍부한 정보량과 나만의 키워드 학습 데이터를 확보합니다.
   - **2단계 (섹터 오분류 핀포인트 보정)**: 라우팅 후 `부동산/아파트`, `현대차/수소차`, `유전자/바이오`, `RWA/블록체인`, `M&A/자사주/밸류업 공시`, `글로벌 반도체` 등 강제 룰을 통해 섹터 억지 분류(예: 반도체 벨트 아파트, HD현대 자사주 공시가 미국으로 튀는 현상)를 100% 교정합니다.
   - **3단계 (동일 섹터 내 Title-Only 중복 축소 & 금액 우선 교체)**: 오직 **'동일한 섹터 내부'에서만** 기사 제목(`Title-Only`) 간 유사도(0.65 기준)를 비교하여 중복 기사를 1개로 좁히며, `29조원`, `1367억` 등 구체적 수주/투자 금액 명시 기사를 우선 대표 기사로 채택합니다.
4. **[Financial Amount Priority Deduplication & Filtering]**:
   - 동일 섹터 중복 검사 시, 구체적인 투자/수주 금액 수치(예: `29조원`, `1367억`, `198억달러`)가 명시된 고가치 기사를 최우선으로 채택하고 단순 시황/중복 기사는 자동 소거합니다.
5. **[Storage & Rendering]**:
