...
from broker_report_agent import get_broker_report_schedules
...
def run_schedule_pipeline():
    ...
    print("📥 12. 당일 목표가 상향 증권사 리포트 수집 중...")
    all_schedules.extend(get_broker_report_schedules())
        ...
        try:
            ...
            for _, row in df_vip_cleaned.iterrows():
                ...
                if re.search(r'\d+월|\d+일', timeline_str):
                    ...
                    card_content = f"""
                    <div style="background: linear-gradient(135deg, #fce7f3 0%, #e0e7ff 100%); border: 1px solid #fbcfe8; border-radius: 20px; padding: 2.5rem; margin-bottom: 3rem; text-align: center; box-shadow: 0 10px 30px rgba(219, 39, 119, 0.08); position: relative; overflow: hidden;">
                        <div style="position: absolute; top: -10px; right: -10px; font-size: 8rem; opacity: 0.05; transform: rotate(15deg);">🚀</div>
                        <span style="background: linear-gradient(90deg, #db2777, #7c3aed); color: white; padding: 0.4rem 1.1rem; border-radius: 30px; font-weight: 800; font-size: 0.9rem; letter-spacing: 0.1em; display: inline-block; margin-bottom: 1.5rem; box-shadow: 0 4px 15px rgba(219, 39, 119, 0.25);">🔥 VIP 핫 모멘텀 ({event_date})</span>
                        <h2 style="font-size: 2rem; font-weight: 800; color: #0f172a; margin-bottom: 1rem; line-height: 1.4;">[{sector}] {issue}</h2>
                        <div style="display: flex; justify-content: center; gap: 2rem; color: #334155; font-size: 1.1rem; margin-bottom: 1.5rem; font-weight: 600;">
                            <span>🗓️ 시기: <strong style="color: #0284c7;">{timeline_str}</strong></span>
                            <span>🎯 수혜주: <strong style="color: #059669;">{stocks}</strong></span>
                        </div>
                    """
                    if vip_link and vip_link != 'nan':
                        card_content += f'<a href="{vip_link}" target="_blank" style="display: inline-block; background: #ffffff; border: 1px solid #cbd5e1; color: #0f172a; text-decoration: none; padding: 0.8rem 2rem; border-radius: 50px; font-weight: 700; box-shadow: 0 2px 8px rgba(15,23,42,0.05); transition: all 0.2s;" onmouseover="this.style.background=\'#4f46e5\'; this.style.color=\'#ffffff\';" onmouseout="this.style.background=\'#ffffff\'; this.style.color=\'#0f172a\';">관련 뉴스 원문 보기 &rarr;</a>'
...
<body>
    <div class="container">
        ...
        <!-- 🧪 로컬 신규 수집 에이전트 4종 (베타테스트) -->
        <div style="background: rgba(99, 102, 241, 0.08); border: 2px dashed rgba(99, 102, 241, 0.4); border-radius: 16px; padding: 1.5rem; margin-bottom: 2rem;">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.2rem; flex-wrap: wrap; gap: 0.5rem;">
                <h2 style="font-size: 1.25rem; font-weight: 700; color: #a5b4fc; display: flex; align-items: center; gap: 0.5rem; margin: 0;">
                    🧪 임시 (베타테스트) <span style="font-size: 0.8rem; background: rgba(99, 102, 241, 0.3); color: #c7d2fe; padding: 0.25rem 0.75rem; border-radius: 50px;">신규 로컬 파이프라인 수집 결과</span>
                </h2>
                <span style="font-size: 0.8rem; color: var(--text-muted); font-weight: 500;">⚡ 100% 로컬 연산 추출 방식</span>
            </div>
            
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(260px, 1fr)); gap: 1rem;">
                <!-- 1. B-1 증권사 리포트 -->
                <div style="background: rgba(15, 23, 42, 0.6); border: 1px solid rgba(255,255,255,0.08); border-radius: 12px; padding: 1.2rem;">
