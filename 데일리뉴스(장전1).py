...
OUTPUT_MD_PATH = f"reports/{date_str}_장전1.md"
...
SECTOR_ANCHOR_KEYWORDS = {
    "반도체": ["반도체", "삼성전자", "SK하이닉스", "HBM", "D램", "낙드", "웨이퍼", "파운드리", "망직소자", "EUV", "CXL", "메모리"],
    ...
    for idx, news in enumerate(collected_news):
        ...
        if max_score >= threshold or (float(max_score) + calculate_momentum_bonus(news)) >= threshold:
            ...
            if not validate_anchor_keyword(news["title"], final_sector):
                final_sector = "기타"
                print(f"⚠️ [앵커 게이트] '{news['title'][:30]}' → 기타로 이동 (사유: [{final_sector}] 압커어 불일치)")
...
def main():
    ...
    OUTPUT_MD_PATH = f"reports/{target_date_str}_장전1.md"
    ...
    if final_report:
        ...
        send_telegram_alert(final_report, end_time.strftime('%Y-%m-%d'), "장전1")
    else:
        print("❌ 보고서 생성 실패.")
