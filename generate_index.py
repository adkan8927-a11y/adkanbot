    ...
    for root, _, files in os.walk(reports_dir):
        for filename in files:
            ...
            if match:
                ...
                # 루트 및 월별 서브폴더 양방향 fallback HTML 복사 (옛/신 텔레그램 링크 모두 404 방지)
                import shutil
                root_html_filepath = os.path.join(reports_dir, html_filename)
                month_subfolder = os.path.join(reports_dir, date_str[:7])
                subfolder_html_filepath = os.path.join(month_subfolder, html_filename)
                ...
                elif html_filepath == root_html_filepath and os.path.exists(html_filepath):
                    try:
                        os.makedirs(month_subfolder, exist_ok=True)
                        shutil.copy2(html_filepath, subfolder_html_filepath)
                    except Exception as copy_err:
                        print(f"Fallback copy to subfolder error for {html_filename}: {copy_err}")
